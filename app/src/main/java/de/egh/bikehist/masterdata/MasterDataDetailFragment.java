...
import de.egh.bikehist.SaveDataService;
...
/**
 ...
 This fragment is either contained in a {@link MasterDataListActivity}
...
public class MasterDataDetailFragment extends Fragment {
			...
			if (intent.hasExtra(SaveDataService.Contract.ACTION)) {
				if (intent.getStringExtra(SaveDataService.Contract.ACTION).equals(SaveDataService.Contract.DeleteMasterDataAction.NAME)) {
					...
					if (type.equals(MasterDataContract.Type.Values.BIKE)) {
						...
						//  Deleted %1$d bike with %2$d event(s).
						Toast.makeText(getActivity(), String.format(getString(R.string.messageDeleteTagsSuccess), nrItems, nrDependent), Toast.LENGTH_LONG).show();
					}
	...
	private boolean hasEvents(Tag tag) {
		...
		Cursor c = cr.query(BikeHistProvider.CONTENT_URI_EVENTS
				, BikeHistProvider.BikeHistContract.QUERY_COUNT_PROJECTION
				, BikeHistProvider.BikeHistContract.Tables.Event.Columns.Name.TAG_ID + "=?"
			...
			/* uri            The URI, using the content:// scheme, for the content to retrieve.
			   ...
			   sortOrder      How to order the rows, formatted as an SQL ORDER BY clause (excluding the ORDER BY itself). Passing null will use the default sort order, which may be unordered.*/
				...
				if (c.getCount() == 1) {
					newEntry = false;
					c.moveToFirst();
					bike = Utils.buildBikeFromCursor(c);
//	      getActivity().getActionBar().setTitle("Edit Bike");
				}
				// Create new Bike
				else {
					bike = new Bike(UUID.randomUUID(), "", "");
				}
			}
			...
			/* uri            The URI, using the content:// scheme, for the content to retrieve.
			   ...
			   sortOrder      How to order the rows, formatted as an SQL ORDER BY clause (excluding the ORDER BY itself). Passing null will use the default sort order, which may be unordered.*/
				Cursor c = cr.query(BikeHistProvider.CONTENT_URI_TAG_TYPES, //
						null, //
						BikeHistProvider.BikeHistContract.Tables.TagType.Columns.Name.ID + "=?", //
						new String[]{getArguments().getString(Contract.Parameter.ITEM_ID)}, //
						null);
				...
				if (c.getCount() == 1) {
					newEntry = false;
					c.moveToFirst();
					tagType = Utils.buildTagTypeFromCursor(c);
				}
				// Create new
				else {
					tagType = new TagType(UUID.randomUUID(), "");
				}
			}
		...
		// Tag
			...
			/* uri            The URI, using the content:// scheme, for the content to retrieve.
			   ...
			   sortOrder      How to order the rows, formatted as an SQL ORDER BY clause (excluding the ORDER BY itself). Passing null will use the default sort order, which may be unordered.*/
				Cursor c = cr.query(BikeHistProvider.CONTENT_URI_TAGS, //
						null, //
						BikeHistProvider.BikeHistContract.Tables.Tag.Columns.Name.ID + "=?", //
						new String[]{getArguments().getString(Contract.Parameter.ITEM_ID)}, //
						null);
				...
				if (c.getCount() == 1) {
					newEntry = false;
					c.moveToFirst();
					tag = Utils.buildTagFromCursor(c);
				}
				// Create new
				else {
					tag = new Tag(UUID.randomUUID(), "", null);
				}
			}
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		//--- Bike ---
		if (bike != null) {
			rootView = inflater.inflate(R.layout.master_data_fragment_item_detail_bike, container, false);
			...
			saveButton.setOnClickListener(new View.OnClickListener() {
				...
				public void onClick(View v) {
					...
					if (name.getText().toString().isEmpty()) {
						Toast.makeText(getActivity(), "NAME missing", Toast.LENGTH_SHORT).show();
						name.requestFocus();
						return;
					}
					if (frameNumber.getText().toString().isEmpty()) {
						Toast.makeText(getActivity(), "Frame Number missing", Toast.LENGTH_SHORT).show();
		...
		//--- Tag Type ---
			...
			rootView = inflater.inflate(R.layout.master_data_fragment_item_detail_tag_type, container, false);
		...
		//--- Tag ---
			...
			rootView = inflater.inflate(R.layout.master_data_fragment_item_detail_tag, container, false);

			name = (TextView) rootView.findViewById(R.id.master_data_fragment_item_detail_tag_tag_name);
			tagTypeSpinner = (Spinner) rootView.findViewById(R.id.master_data_fragment_item_detail_tag_tag_type_spinner);
	...
	private void saveTag() {
		//update entity
		ContentResolver cr = getActivity().getContentResolver();
		String where = BikeHistProvider.BikeHistContract.Tables.Tag.Columns.Name.ID + "=?";
		String[] args = {tag.getId().toString()};

		Cursor c = cr.query(BikeHistProvider.CONTENT_URI_TAGS, null, where, args, null);
		if (c.getCount() == 0) {
			if (cr.insert(BikeHistProvider.CONTENT_URI_TAGS, Utils.buildTagContentValues(tag)) != null) {
			...
			ContentValues cv = Utils.buildTagContentValues(tag);
	...
	private void saveTagType() {
		//update entity
		ContentResolver cr = getActivity().getContentResolver();
		String where = BikeHistProvider.BikeHistContract.Tables.TagType.Columns.Name.ID + "=?";
		String[] args = {tagType.getId().toString()};

		Cursor c = cr.query(BikeHistProvider.CONTENT_URI_TAG_TYPES, null, where, args, null);
		if (c.getCount() == 0) {
			if (cr.insert(BikeHistProvider.CONTENT_URI_TAG_TYPES, Utils.buildTagTypeContentValues(tagType)) != null) {
				Toast.makeText(getActivity(), "Saved", Toast.LENGTH_SHORT).show();
				mCallbacks.onChanged();
			} else {
				Log.e(TAG, "Insert failed !");
			...
			ContentValues cv = Utils.buildTagTypeContentValues(tagType);
	...
	private void saveBike() {
		//update entity
		ContentResolver cr = getActivity().getContentResolver();
		String where = BikeHistProvider.BikeHistContract.Tables.Bike.Columns.Name.ID + "=?";
		String[] args = {bike.getId().toString()};

		Cursor c = cr.query(BikeHistProvider.CONTENT_URI_BIKES, null, where, args, null);
		if (c.getCount() == 0) {
			if (cr.insert(BikeHistProvider.CONTENT_URI_BIKES, Utils.buildBikeContentValues(bike)) != null) {
				Toast.makeText(getActivity(), "Saved", Toast.LENGTH_SHORT).show();
				mCallbacks.onChanged();
			} else {
				Log.e(TAG, "Insert failed !");
			...
			ContentValues cv = Utils.buildBikeContentValues(bike);
	...
	/**
	 ...
	 implement. This mechanism allows activities to be notified of item changed
	 tu update their views.
	 */
	public interface Callbacks {
		/**
		 Callback for when the item was changed.
		 */
		public void onChanged();
	}

	/** Paramters for consuming the Fragment. */
